...
#define FRAMETYPE_PROBE_REQUEST		0x40	//add by kaixi fan, 20140915
...
uint32_t frames_proberequest = 0;
...
int curl_sendMactoServer(char *url)
{
	...
	if (CURLE_OK != return_code)
	{
		printf("send router myself mac to server failed.\n");
	}
...
int main(int argc, char **argv)
{
	...
	char macAddr[MACHDADDR_LEN];	//add by kaixi fan, 20190915
	uint8_t *ieee802_11pkt;	//add by kaixi fan, 20190915
	ssize_t pktlen;
	...
	*/
	...
	while (1)
	{
		...
		msg("3\n");
		frametype = *(uint8_t *)(pktbuf + le16(rhdr->it_len));	//add by kaixi fan
		...
		*/
		...
		/*add by kaixi fan, 20140915, skip packet not probe request*/
		if((frametype & FRAMETYPE_MASK) != FRAMETYPE_PROBE_REQUEST)
		{
			msg("4\n");
			frames_filtered++;
			continue;
		...
		msg(" %d, mac = 0x%02x:%02x:%02x:%02x:%02x:%02x \n",
				macAddr[0], macAddr[1], macAddr[2], macAddr[3], macAddr[4], macAddr[5]);

		if(!sendRouterMacOpenId_to_Server(macAddr))
			continue;
